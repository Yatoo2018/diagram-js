...
SpaceTool.prototype.calculateAdjustments = function(elements, axis, delta, start) {
  ...
  forEach(elements, function(element) {
    ...
    if ((delta > 0 && shapeStart > start) || (delta < 0 && shapeEnd < start)) {
      ...
      if (element.label && !includes(movingShapes, element.label)) {

        // shape to be moved, move label
        movingShapes.push(element.label);
      }
  ...
  forEach(movingShapes, function(shape) {
    shape.attachers && forEach(shape.attachers, function(attacher) {
      if (!includes(movingShapes, attacher)) {
        movingShapes.push(attacher);

        if (attacher.label && !includes(movingShapes, attacher.label)) {

          // attacher to be moved, move label
          movingShapes.push(attacher.label);
        }
      }
    });
  });
  ...
  forEach(attachers, function(attacher) {
    if (includes(allShapes, attacher.host) && !includes(movingShapes, attacher)) {
      movingShapes.push(attacher);
  ...
  forEach(connections, function(connection) {
    ...
    if (includes(allShapes, source)
      ...
      && connection.label
      && !includes(movingShapes, connection.label)) {

      // connection to be moved, move label
      movingShapes.push(connection.label);
    }
  });
